...
export const BackToTop: React.FC = () => {
  ...
  return (
    <button
      onClick={scrollToTop}
      className={`fixed bottom-8 right-8 z-[60] w-12 h-12 bg-white dark:bg-slate-800 text-[#06B6D4] rounded-full shadow-2xl flex items-center justify-center transition-all duration-500 border border-slate-100 dark:border-slate-700 hover:scale-110 hover:bg-[#06B6D4] hover:text-white active:scale-95 ${
...
export const SideNavigation: React.FC<{ items: SideNavItem[] }> = ({ items }) => {
  ...
  return (
    <nav className="fixed left-4 top-1/2 -translate-y-1/2 z-[40] hidden lg:flex flex-col gap-4 bg-white/40 dark:bg-slate-900/40 backdrop-blur-md p-3 rounded-2xl border border-white/20 dark:border-slate-800 shadow-xl">
      {items.map((item, index) => (
        ...
        >
          <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
            activeId === item.id 
              ? 'bg-[#06B6D4] scale-125 ring-4 ring-[#06B6D4]/20' 
...
// --- RGBCard Component for Student Achievements ---
export const RGBCard: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => {
  return (
    <div className={`relative group p-[2px] rounded-2xl overflow-hidden ${className} shadow-lg hover:shadow-2xl transition-shadow duration-300`}>
      {/* The RGB Border Gradient - visible on hover via opacity */}
      <div className="absolute inset-0 bg-gradient-to-r from-red-500 via-green-500 to-blue-500 bg-[length:200%_200%] opacity-0 group-hover:opacity-100 transition-opacity duration-300 animate-rgb-border z-0" />
      
      {/* The Card Content Background - slightly smaller to show border */}
      <div className="relative z-10 bg-white dark:bg-slate-900 rounded-[14px] h-full w-full overflow-hidden border border-slate-100 dark:border-slate-800 group-hover:border-transparent">
        {children}
      </div>
    </div>
  );
